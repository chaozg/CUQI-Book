...
(iii) if small perturbations of the data can only produce small perturbations of the solution. Nowadays we refer to these as the three \textit{Hadamard conditions}.

A problem, then, is __ill posed__ if it violates one or more of these Hadamard conditions.Let us discuss this using a linear matrix problem $A\,x = b$, where the right-hand side vector $b$ represents the data, the vector $x$ represents the solution, and the matrix $A$ represents the forward model. These violations can also occur for nonlinear problems.
